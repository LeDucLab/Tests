...
if st.button("Nächste Frage", key="Q2"):
    for question_data in question_data_2:
        ...
        st.write(question_data['Question'])
        st.session_state.user_answer_2 = st.radio("Ihre Antwort:", options=question_data['Options'], key="user_answer_2")
        if st.button("Einreichen", key="A2"):
            if st.session_state.user_answer_2 == question_data['Answer']:
                st.success("Korrekt!")
                st.markdown(f'<img src="{question_data["CorrectImageURL"]}" alt="Korrekt" width="100%">', unsafe_allow_html=True)
            else:
                st.warning("Falsch! Versuchen Sie nochmal.")
