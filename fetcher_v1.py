...
if st.button("Retrieve ACMG Information"):
    ...
    try:
        ...
        hom_count= variant.get("hom_count_reference_population")
        if af:
            if hom_count == 0:
                gnomad_text = (
                    f"In der Populationsdatenbank gnomAD v4.1.1 beträgt die Allelfrequenz {af} " f"und es wurden keine homozygoten Träger beobachtet."
                )
            else:
                gnomad_text = (
                    f"In der Populationsdatenbank gnomAD v4.1.1 beträgt die Allelfrequenz {af} " f"und es wurden {hom_count} homozygote Träger beobachtet."
                )
        else:
            gnomad_text = (
